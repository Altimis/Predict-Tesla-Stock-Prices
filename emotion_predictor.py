...
class EmotionPredictor:
    ...
    def predict_probabilities(self, tweet):
        indice = self._tweet_to_indices(tweet)
        #indice = np.array(indice)
        prediction = self.model.predict(indice, verbose=False)

        #df = pd.DataFrame({'Tweet': tweets})
        df = {}
        for emotion, values in zip(self.class_values, prediction.T):
            df[emotion] = values[0]
        return df
